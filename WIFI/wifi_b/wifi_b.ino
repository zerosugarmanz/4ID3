...
void loop() {
  ...
  client.publish("4ID3_Group8/temperature", String(dhtTempEvent.temperature).c_str());
  delay(100);
  client.publish("4ID3_Group8/humidity", String(dhtHumEvent.relative_humidity).c_str());
  delay(100);
  client.publish("4ID3_Group8/pressure", String(bmpEvent.pressure).c_str());
  delay(100);
  client.publish("4ID3_Group8/light", String(lux).c_str());
